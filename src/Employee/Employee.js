...
export default function Employee() {
  ...
  const deleteData = (id) => {
    axios.delete(`http://localhost:4000/delete/users/${id}`).then((res) => {
      addDataList();
    });
  };
  ...
  const updateData = async () => {
    ...
    axios
      .put(`http://localhost:4000/api/users/${id}`, userDetails)
      .then((res) => {
        addDataList(id);
        handleOff();
      });
  ...
  const handleEditSubmit = async () => {
    ...
    await axios
      .post(`http://localhost:4000/api/create`, userDetails)
      .then((res) => {
        addDataList();
        handleOff();
      });
