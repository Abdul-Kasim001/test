...
function Client() {
  ...
  const handleFormSubmit = async () => {
    ...
    await axios
      .post(`http://localhost:8090/api/create`, userDetails)
      .then((resp) => {
        addClient();
        handleoff();
      });
  ...
  const addClient = () => {
    axios.get(`http://localhost:8090/api/get`).then((test) => {
  ...
  const deleteClientdata = (id) => {
    axios.delete(`http://localhost:8090/delete/users/${id}`).then((test) => {
      addClient();
    });
  };
  ...
  const ClientEdit = async () => {
    ...
    axios
      .put(`http://localhost:8090/api/users/${id}`, updatedetails)
      .then((test) => {
        addClient();
        handleoff();
      });
          ...
          <div>
            ...
            >
              <Box sx={style}>
                ...
                <Tooltip title="Close">
                  <Button className="viewclose" onClick={close}>
                    x
                  </Button>
                </Tooltip>
              </Box>
